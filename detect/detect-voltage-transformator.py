...
# Загрузка модели
model = YOLO(r"detect-voltage-transformator.pt")
    ...
    with tqdm(total=total_frames, desc=f"Обработка {video_name}", unit=" кадр", leave=False) as pbar:
        ...
        while True:
            ...
            for i, (class_id, box, conf) in enumerate(zip(classes, boxes, confidences)):
                ...
                # Заменяем 'gauges' на 'voltage-transformator'
                if class_name == 'gauges':
                    class_name = 'voltage-transformator'
                elif class_name == "voltage-transformer":
                    class_name = 'voltage-transformator'
...
with tqdm(total=len(video_files), desc="Обработка видео voltage-transformator", unit=" видео") as overall_pbar:
